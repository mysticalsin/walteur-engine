...
selftest() {
  ...
  _nojq_path() { # $1=tmp -> echoes a PATH dir with no jq
    ...
    for t in bash sh env grep egrep sed awk gawk sort head tail cat tr cut wc date mkdir rm rmdir ln chmod find dirname basename cp mv touch mktemp printf true false test; do
  ...
  run_one() { # $1=label $2=want-rc $3=setup-fn (sets up $tmp, may export OSV_FIXTURE/WALTEUR_OSV)
    ...
    SELF_FIXTURE=""; SELF_MODE="on"; SELF_ENDPOINT=""; SELF_NOJQ=""; SELF_VERDICT=""
    ...
    local run_path="$PATH"
    [ -n "$SELF_NOJQ" ] && run_path="$(_nojq_path "$tmp")"
    env -i PATH="$run_path" HOME="${HOME:-/tmp}" TMPDIR="${TMPDIR:-/tmp}" \
        WALTEUR_ROOT="$tmp" WALTEUR_OSV="$SELF_MODE" "${osv_env[@]}" \
        bash "$SELF" >/dev/null 2>&1
    rc=$?
    set -e
    local okv=1; [ "$rc" -eq "$2" ] || okv=0
    if [ -n "$SELF_VERDICT" ]; then
      local gv; gv="$(jq -r '.verdict // ""' "$tmp/walteur-kit/osv-report.json" 2>/dev/null)"
