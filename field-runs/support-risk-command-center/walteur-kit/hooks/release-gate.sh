...
SH
  ...
  jq -e '.verdict == "FAIL" and .violations == 1 and .details.grype.high_or_critical == 1' "$tmp/walteur-kit/release-report.json" >/dev/null 2>&1
  ...
  mkdir -p "$tmp/walteur-kit"
  make_dockerfile "$tmp"
  WALTEUR_ROOT="$tmp" WALTEUR_RELEASE=off bash "$0" >/dev/null 2>&1
