...
DIR="${1:-}"
if [ -z "$DIR" ]; then
  echo "WALTEUR anti-slop-ui SKIP — no directory argument. Usage: anti-slop-ui.sh <dir>" >&2
...
while IFS= read -r f; do
  ...
  \( -path '*/node_modules/*' -o -path '*/.git/*' -o -path '*/dist/*' -o -path '*/build/*' -o -path '*/.next/*' -o -path '*/coverage/*' \) -prune -o \
