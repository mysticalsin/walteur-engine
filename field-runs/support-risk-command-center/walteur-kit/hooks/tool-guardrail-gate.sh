...
#       compensate, abort}, AND evidence_ref on every band (declaration must be wired, not asserted).
      ...
      ( ( ($t.side_effect_class == "write_irreversible")
        ...
        | if $dangerous
            then (
              ...
              ( ($t.error_path.on_fatal) as $of
                | if (($of|type) == "string"
                       and ((["halt","escalate","rollback","compensate","abort"]) | index($of|ascii_downcase)))
                    then empty
                    else "G5::tool \($tn): dangerous/external tool REQUIRES on_fatal to halt/escalate/rollback/compensate/abort (got \($of|tojson))" end ),
