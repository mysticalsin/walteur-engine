// server.mjs — HTTP surface over core.mjs with DENY-BY-DEFAULT tenant isolation for the API-KEY VAULT.
//
...
// isolation is NOT reimplemented here; cross-tenant access is denied there (403 on write, 404/null on read).
//
// THE DOMAIN SECRET RULE: a freshly minted RAW key is returned exactly ONCE — in the 201 response of
// POST /keys and the 200 response of POST /keys/:id/rotate. It is NEVER stored, NEVER returned by GET /keys,
// and is REDACTED out of every log line. The store and data.json hold sha256 HASHES only.
//
// Logs are structured JSON to stdout with the Authorization header, any token, and any raw `key` value
// REDACTED to "***". Persistence mirrors the store to data.json after every mutation (atomic tmp+rename);
// data.json holds only hashes + metadata + audit rows — never a raw key or a bearer token.
//
...
const DATA_FILE = process.env.VAULT_DATA_FILE || path.join(__dirname, 'data.json');
const BODY_LIMIT = 1024 * 1024; // 1MB cap

// Structured JSON log to stdout. REDACTS any credential-bearing field to "***" so neither a bearer token
// NOR a freshly minted raw key can ever be written to a log line, even by accident.
const REDACT_KEYS = new Set(['authorization', 'token', 'bearer', 'x-token', 'tenant_tokens', 'key', 'apikey', 'api_key', 'secret']);
...
// same volume). The snapshot is HASHES ONLY by construction — a raw key can never reach this file. Errors
// are logged, never thrown — a persistence hiccup must not break a live request.
...
// createServer(store, { persistOnMutate }) -> an UNLISTENED http.Server, so tests can server.listen(0) on an
// ephemeral port. The tenant token map is resolved per-request from env, so a test can set
...
export function createServer(store = createStore(), { persistOnMutate = false } = {}) {
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      // GET /keys  (tenant-scoped list of the caller's OWN keys — METADATA ONLY, never a raw key)
      if (method === 'GET' && url.pathname === '/keys') {
        send(res, 200, store.listKeys(tenantId));
        return finish(200, tenantId);
      }

      // POST /keys  { label }  -> issue a key the caller OWNS; returns the RAW key ONCE in the 201 body
      if (method === 'POST' && url.pathname === '/keys') {
        ...
        const label = body.value && body.value.label;
        if (!label || !String(label).trim()) { send(res, 400, { error: 'label required' }); return finish(400, tenantId); }
        try {
          const issued = store.createKey(tenantId, label); // { ...metadata, key } — key shown ONCE
          save();
          send(res, 201, issued);
      ...
      // GET /audit  (tenant-scoped audit trail — never a raw key or hash of one)
      ...
      // POST /keys/:id/rotate  -> mint a NEW raw key for an OWNED key; returns the new RAW key ONCE
      const rotateMatch = /^\/keys\/([^/]+)\/rotate$/.exec(url.pathname);
      if (rotateMatch && method === 'POST') {
        const id = decodeURIComponent(rotateMatch[1]);
        try {
          const rotated = store.rotateKey(tenantId, id); // throws 'forbidden' / 'revoked'
          save();
          send(res, 200, rotated);
          return finish(200, tenantId);
        } catch (e) {
          if (e && /revoked/.test(e.message)) { send(res, 409, { error: 'revoked' }); return finish(409, tenantId); }
          return handleOwnershipError(e, res, finish, tenantId);
        }
      }

      // /keys/:id  — GET one OWNED key's metadata, DELETE to REVOKE it
      const keyMatch = /^\/keys\/([^/]+)$/.exec(url.pathname);
      if (keyMatch) {
        const id = decodeURIComponent(keyMatch[1]);
        if (method === 'GET') {
          const rec = store.getKey(tenantId, id); // null on cross-tenant/absent — no leak
          if (!rec) { send(res, 404, { error: 'not found' }); return finish(404, tenantId); }
          send(res, 200, rec);
          return finish(200, tenantId);
        }
        if (method === 'DELETE') {
          try {
            const rev = store.revokeKey(tenantId, id); // throws 'forbidden' on cross-tenant/absent
            save();
            send(res, 200, rev);
            return finish(200, tenantId);
          } catch (e) {
...
// core.mjs throws 'forbidden' for BOTH a cross-tenant access AND an absent id (both deny-by-default). To
// return an honest status without leaking another tenant's existence, a rotate/revoke of a key the caller
// does not OWN is 'forbidden' => 403. We cannot tell absent vs cross-tenant without leaking, so the
...
// ── CLI entrypoint: listen on PORT (default 8203) when run directly ───────────────────────────────
