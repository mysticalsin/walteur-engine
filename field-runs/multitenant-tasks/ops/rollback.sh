#!/usr/bin/env bash
# ops/rollback.sh — REAL blue-green rollback driver for the multitenant-tasks service.
#
...
#
#   --check   PRE-FLIGHT (the rollback PROOF the zero-downtime-cutover-gate re-executes). It does NOT mutate
#             anything. It proves the rollback is ACTUALLY POSSIBLE by verifying a prior-version marker:
...
# Resolve the directory THIS script lives in, so it works regardless of the caller's CWD (the gate runs it
# from $ROOT, a human may run it from ops/).
SELF_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
VERSIONS="$SELF_DIR/versions.json"
HEALTH_URL="${HEALTH_URL:-http://localhost:8137/health}"
...
verify_prior_marker() {
  ...
  # node parses + validates the invariant and prints the previous version, or exits non-zero.
  node -e '
...
case "$cmd" in
  ...
  --execute)
    ...
    # In a live env this is where the alias flip happens (kubectl/helm/lb API). Then assert health.
