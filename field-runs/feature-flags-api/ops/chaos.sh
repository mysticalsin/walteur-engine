#!/usr/bin/env bash
# ops/chaos.sh — REAL chaos / game-day drill for the multi-tenant feature-flags HTTP surface.
#
...
start_server() {
  ...
  FLAGS_DATA_FILE="$ROOT/data.chaos.json" PORT="$PORT" node "$SERVER" >>"$LOG" 2>&1 &
  echo "$!"
}
...
write_report() {
  ...
  node -e '
    ...
    const report = {
      ...
      notes: "Primary started + steady on GET /health, hard-killed (taskkill/kill -9), fresh instance restarted and re-probed healthy. recovery_seconds = kill -> first healthy green probe (observed)."
    };
