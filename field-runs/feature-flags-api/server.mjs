// server.mjs — HTTP surface over core.mjs with DENY-BY-DEFAULT tenant isolation for FEATURE FLAGS.
//
...
// tmp+rename) and reloaded on boot. data.json holds only flags + audit rows — never any secret.
//
...
const DATA_FILE = process.env.FLAGS_DATA_FILE || path.join(__dirname, 'data.json');
...
export function createServer(store = createStore(), { persistOnMutate = false } = {}) {
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      // GET /flags  (tenant-scoped list of the caller's OWN flags)
      if (method === 'GET' && url.pathname === '/flags') {
        send(res, 200, store.getFlags(tenantId));
        return finish(200, tenantId);
      }

      // POST /flags  { key, value }  -> create/update a flag the caller OWNS
      if (method === 'POST' && url.pathname === '/flags') {
        ...
        const key = body.value && body.value.key;
        const value = body.value && body.value.value;
        if (!key || !String(key).trim()) { send(res, 400, { error: 'key required' }); return finish(400, tenantId); }
        try {
          const flag = store.setFlag(tenantId, key, value);
          save();
          send(res, 201, flag);
      ...
      // /flags/:key  — GET one OWNED flag, DELETE one OWNED flag
      const flagMatch = /^\/flags\/([^/]+)$/.exec(url.pathname);
      if (flagMatch) {
        const key = decodeURIComponent(flagMatch[1]);
        if (method === 'GET') {
          const flag = store.getFlag(tenantId, key); // null on cross-tenant/absent — no leak
          if (!flag) { send(res, 404, { error: 'not found' }); return finish(404, tenantId); }
          send(res, 200, flag);
          return finish(200, tenantId);
        }
        if (method === 'DELETE') {
          try {
            store.deleteFlag(tenantId, key); // throws 'forbidden' on cross-tenant/absent
            save();
...
// core.mjs throws 'forbidden' for BOTH a cross-tenant access AND an absent key (both deny-by-default).
// To return an honest status without leaking another tenant's existence, a write to a flag the caller does
...
// ── CLI entrypoint: listen on PORT (default 8191) when run directly ───────────────────────────────
