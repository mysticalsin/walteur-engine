...
- [ ] Completeness — no silent mocks: every external integration is live-wired-and-proven, signed-deferred (owner+ticket), or a time-boxed prototype. Evidence: `report:walteur-kit/integration-proof-report.json`
- [ ] Completeness — measured, not claimed: for UI builds, Lighthouse categories + axe ran and met thresholds. Evidence: `report:walteur-kit/measured-quality-report.json`
- [ ] Completeness — tests at every layer: logic + component + E2E (UI) executed with exit 0. Evidence: `report:walteur-kit/test-layer-coverage-report.json`
- [ ] Security baseline addressed — the 11-point production checklist (privacy/GDPR · RLS · auth failure-path tests · security headers · OWASP review · server-side validation · no data leaks · no frontend secrets · rate limits · CAPTCHA+CORS · safe error messages); each verified, signed-deferred, or N/A. Evidence: `report:walteur-kit/security-baseline-report.json`
- [ ] Revenue integrity (if payments) — webhook signature verification + event-id idempotency; money-moving calls carry idempotency keys. Evidence: `report:walteur-kit/billing-integrity-report.json`
- [ ] Audit trail (if auth) — tamper-evident, ≥365-day-retained log of privileged actions capturing actor/tenant/ts/action (SOC2 CC7.2/7.3). Evidence: `report:walteur-kit/audit-trail-report.json`
...
## 8. Brownfield Upgrade (§2.6 — only when upgrading an existing app; greenfield: `N/A - greenfield build`)

- [ ] Brownfield signal recorded: `walteur-kit/preflight-signals.json` carries `is_brownfield:true`.
- [ ] Intent recovered: `walteur-kit/INTENT.md` exists and passes `intent-reconstruction-gate.sh` — what-it-is, original goal, used-for, users, and a claims ledger where every claim is labeled `confirmed`/`inferred`/`unknown` with file:line evidence (or a non-stub `PRD.md` documents intent for a WALTEUR-built app). Evidence: `report:walteur-kit/intent-reconstruction-report.json`
- [ ] Open questions from the reconstruction (`INTENT.md` `open_questions[]`) are resolved with Tony, or the dependent upgrade tasks are explicitly deferred.
- [ ] Baseline captured BEFORE the first edit: `walteur-kit/baseline.json` passes `baseline-capture-gate.sh` — build status, existing tests, numeric 8-dim scores, and a characterization/golden-master net that locks current observable behavior. Evidence: `report:walteur-kit/baseline-report.json`
- [ ] Every upgrade task names the dimension it Lifts (`from→to` vs baseline), its Tier (`refine-in-place`/`modernize`/`re-architect`), and a non-regression AC. Evidence: `report:walteur-kit/upgrade-backlog.json`
- [ ] Cross-cutting symbol edits recorded callers-checked in `walteur-kit/blast-radius.json` (passes `blast-radius-gate.sh`).
- [ ] Non-regression PROVEN: `walteur-kit/non-regression.json` passes `non-regression-gate.sh` — after ≥ before on EVERY baseline dimension, golden-master net green, and every behavior change cites a signed ADR. Evidence: `report:walteur-kit/non-regression-report.json`
- [ ] No silent behavior change: every observable behavior that changed is named and carries a signed ADR (`walteur-kit/adr/*`); behavior is otherwise preserved.
