...
# `pipefail` matters here: proof/lockfile parsing runs through pipelines, and without it a
# failing producer is masked by a successful consumer, so the gate can read green on input it
# never parsed. 178 of the other hooks already set it; this brings the prologue to the norm.
set -uo pipefail
