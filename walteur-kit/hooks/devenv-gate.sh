...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "devenv-gate - ZERO-DEP hard gate on a reproducible developer environment."
  printf '%s\n' "usage: bash devenv-gate.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/devenv-report.json - fix recipes: walteur-kit/REMEDIATION.md (## devenv-gate)"
  printf '%s\n' "bypass: WALTEUR_DEVENV=off (recorded, not free)"
  exit 0 ;;
esac
