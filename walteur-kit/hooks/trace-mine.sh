...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "trace-mine - the SELF-IMPROVING offline trace miner (HALOs one net-new, done WALTEURs flat-file way)."
  printf '%s\n' "usage: bash trace-mine.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/trace-mine-report.json - fix recipes: walteur-kit/REMEDIATION.md (## trace-mine)"
  printf '%s\n' "bypass: WALTEUR_TRACEMINE=off (recorded, not free)"
  exit 0 ;;
esac
