...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "scoreboard-gate - typed eight-dimension score contract gate."
  printf '%s\n' "usage: bash scoreboard-gate.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/scoreboard-report.json - fix recipes: walteur-kit/REMEDIATION.md (## scoreboard-gate)"
  printf '%s\n' "bypass: WALTEUR_SCOREBOARD=off (recorded, not free)"
  exit 0 ;;
esac
