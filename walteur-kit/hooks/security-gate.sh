...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "security-gate - honest detect-or-loud-SKIP secret/dependency/SAST gate."
  printf '%s\n' "usage: bash security-gate.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/security-report.json - fix recipes: walteur-kit/REMEDIATION.md (## security-gate)"
  printf '%s\n' "bypass: WALTEUR_SECURITY=off (recorded, not free)"
  exit 0 ;;
esac
...
selftest() {
  ...
  for t in bash jq mktemp date mkdir rm chmod cat; do
    if ! have "$t"; then
  ...
  make_core_path() {
    dst="$1"
    mkdir -p "$dst"
    # exec-wrappers, NOT `ln -sf`: on Git-Bash/MSYS `ln` silently degrades to a COPY, and a
    # copied .exe can no longer reach the msys runtime DLLs that live beside the original once
    # PATH is EXCLUSIVE (see the no-scanners fixture below) — every hermetic run died rc=127
    # before the report was written, proven 2026-08-27. A wrapper leaves the real binary where
    # its runtime is, so the exclusive PATH stays real on Windows as well as on Linux/macOS.
    for t in bash jq mktemp date mkdir rm; do
      printf '#!%s\nexec "%s" "$@"\n' "$(command -v bash)" "$(command -v "$t")" > "$dst/$t"
      chmod +x "$dst/$t"
    done
  }
  make_fake_gitleaks() {
...
SH
  ...
  # PATH must be EXCLUSIVE to the core toolset here (not prepended to the real $PATH) --
  # otherwise a real gitleaks/npm/osv-scanner/semgrep installed on the host machine
  # (e.g. /opt/homebrew/bin/gitleaks) is still reachable and this "no scanners" fixture
  # silently isn't one.
...
if have gitleaks; then
  ran=$((ran+1))
  # FILE-mode scan (--no-git): the gate certifies the BUILD TREE, not repo history; history-mode on a
  # tree without VCS metadata either errors (misread as findings) or scans nothing (false-green on
  # real secrets) — both proven 2026-07-03. --exit-code 9 separates findings(9) from tool errors.
  # Infrastructure allowlist: walteur-kit/hooks + .claude/hooks carry INTENTIONAL dummy secrets as
  # selftest teaching fixtures (stripe test tokens etc.); app/source stays fully scanned.
  GLCFG="$(mktemp "${TMPDIR:-/tmp}/gitleaks-cfg.XXXXXX")" || GLCFG=""
  GL_ARGS=""
  if [ -n "$GLCFG" ]; then
    printf '%s\n' '[extend]' 'useDefault = true' '[allowlist]' 'paths = ["walteur-kit/hooks/.*", "\\.claude/hooks/.*"]' > "$GLCFG"
    GL_ARGS="-c $GLCFG"
  fi
  gitleaks detect --source "$ROOT" --no-git --no-banner --redact --exit-code 9 $GL_ARGS >/dev/null 2>"$TMP"; gl_rc=$?
  if [ "$gl_rc" -eq 0 ]; then
    echo "  ok   — gitleaks: no secrets found." >&2
    add gitleaks '{"verdict":"PASS","tool":"gitleaks","rule":"any-finding=fail"}'
  elif [ "$gl_rc" -eq 9 ]; then
    echo "  FAIL — gitleaks: secret(s) detected." >&2
    violations=$((violations+1))
    add gitleaks '{"verdict":"FAIL","tool":"gitleaks","rule":"any-finding=fail"}'
  else
    # Tool ERROR (not findings): fail-closed — cannot prove the tree is clean — but say so honestly
    # instead of claiming secrets were detected (that misdiagnosis burned 6 fixture cases).
    echo "  FAIL — gitleaks: runtime error (rc=$gl_rc, not a findings verdict) — cannot prove clean; failing closed. stderr: $(head -c 200 "$TMP" 2>/dev/null)" >&2
    violations=$((violations+1))
    add gitleaks '{"verdict":"FAIL","tool":"gitleaks","rule":"tool-error-fail-closed"}'
  fi
else
