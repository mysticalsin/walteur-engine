...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "edge-protection - HARD gate on -14 Layer 9 (Rate Limiting) + Layer 10 (Caching & CDN)."
  printf '%s\n' "usage: bash edge-protection.sh [<dir>|--help]   (no --selftest: this hook has none)"
  printf '%s\n' "report: walteur-kit/edge-report.json - fix recipes: walteur-kit/REMEDIATION.md (## edge-protection)"
...
# positional <dir> — so `edge-protection.sh --selftest` was read as a DIRECTORY NAME, reported
# "no/invalid directory argument" and exited 0. An operator asking for a selftest got a green that
# proved nothing. Dash-led tokens are never valid directories; fail loudly instead.
case "${1:-}" in
  -*) printf '%s\n' "edge-protection: unrecognized option '$1' — \$1 is the positional <dir>; this hook implements --help only and has no selftest." >&2
      printf '%s\n' "usage: bash edge-protection.sh [<dir>|--help]" >&2
      exit 2 ;;
esac
...
has_signal() { # $1 = extended regex → 0 if any file matches
  [ -n "$ALL_FILES" ] && [ -n "$(printf '%s\n' "$ALL_FILES" | xargs grep -lEi "$1" 2>/dev/null | head -n1)" ]
}
