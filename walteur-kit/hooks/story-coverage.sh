...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "story-coverage - HARD gate on Storybook story completeness. One finding = fail (exit 2). Clean = exit 0."
  printf '%s\n' "usage: bash story-coverage.sh [<dir>|--help]   (no --selftest: this hook has none)"
  printf '%s\n' "report: walteur-kit/story-report.json - fix recipes: walteur-kit/REMEDIATION.md (## story-coverage)"
...
# positional <dir> — so `story-coverage.sh --selftest` was read as a DIRECTORY NAME, reported
# "'--selftest' is not a directory (nothing to scan)" and exited 0. An operator asking for a
# selftest got a green that proved nothing. Dash-led tokens are never valid dirs; fail loudly.
case "${1:-}" in
  -*) printf '%s\n' "story-coverage: unrecognized option '$1' — \$1 is the positional <dir>; this hook implements --help only and has no selftest." >&2
      printf '%s\n' "usage: bash story-coverage.sh [<dir>|--help]" >&2
      exit 2 ;;
esac
