...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "build-contract-lint - typed intake gate for any enterprise build."
  printf '%s\n' "usage: bash build-contract-lint.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/build-contract-report.json - fix recipes: walteur-kit/REMEDIATION.md (## build-contract-lint)"
  printf '%s\n' "bypass: WALTEUR_BUILD_CONTRACT_GATE=off (recorded, not free)"
  exit 0 ;;
esac
