...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "prompt-refinement-gate - Improve-this-prompt before planning/building."
  printf '%s\n' "usage: bash prompt-refinement-gate.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/prompt-refinement-report.json - fix recipes: walteur-kit/REMEDIATION.md (## prompt-refinement-gate)"
  printf '%s\n' "bypass: WALTEUR_PROMPT_REFINEMENT=off (recorded, not free)"
  exit 0 ;;
esac
