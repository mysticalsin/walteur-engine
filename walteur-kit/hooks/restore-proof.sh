...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "restore-proof - honest detect-or-loud-SKIP backup-restore proof gate."
  printf '%s\n' "usage: bash restore-proof.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/restore-report.json - fix recipes: walteur-kit/REMEDIATION.md (## restore-proof)"
  printf '%s\n' "bypass: WALTEUR_RESTORE_PROOF=off (recorded, not free)"
  exit 0 ;;
esac
