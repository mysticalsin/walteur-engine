...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "migration-lint - honest zero-dep migration-safety gate + detect-or-loud-SKIP linters."
  printf '%s\n' "usage: bash migration-lint.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/migration-report.json - fix recipes: walteur-kit/REMEDIATION.md (## migration-lint)"
  printf '%s\n' "bypass: WALTEUR_MIGRATION=off (recorded, not free)"
  exit 0 ;;
esac
