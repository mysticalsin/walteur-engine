...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "contract-gate - ZERO-DEP hard gate on API contracts, with a spectral-lint upgrade."
  printf '%s\n' "usage: bash contract-gate.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/contract-report.json - fix recipes: walteur-kit/REMEDIATION.md (## contract-gate)"
  printf '%s\n' "bypass: WALTEUR_CONTRACT=off (recorded, not free)"
  exit 0 ;;
esac
