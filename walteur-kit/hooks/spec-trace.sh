...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "spec-trace - ZERO-DEP HARD gate on PLAN.md requirement traceability."
  printf '%s\n' "usage: bash spec-trace.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/spec-trace-report.json - fix recipes: walteur-kit/REMEDIATION.md (## spec-trace)"
  printf '%s\n' "bypass: WALTEUR_SPECTRACE=off (recorded, not free)"
  exit 0 ;;
esac
