...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "sbom-gate - first-class dependency inventory / SBOM proof."
  printf '%s\n' "usage: bash sbom-gate.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/sbom-report.json - fix recipes: walteur-kit/REMEDIATION.md (## sbom-gate)"
  printf '%s\n' "bypass: WALTEUR_SBOM=off (recorded, not free)"
  exit 0 ;;
esac
...
JSON
  ...
  PATH="$tmp/bin:$PATH" WALTEUR_ROOT="$tmp" bash "$SELF_PATH" >/dev/null 2>&1; rc=$?
...
if have syft; then
  tmp_sbom="$(mktemp "${TMPDIR:-/tmp}/walteur-sbom.XXXXXX")" || {
