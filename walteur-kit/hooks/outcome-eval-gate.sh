...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "outcome-eval-gate - independent outcome evaluator contract."
  printf '%s\n' "usage: bash outcome-eval-gate.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/outcome-eval-report.json - fix recipes: walteur-kit/REMEDIATION.md (## outcome-eval-gate)"
  printf '%s\n' "bypass: WALTEUR_OUTCOME_EVAL=off (recorded, not free)"
  exit 0 ;;
esac
