...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "nfr-lint - Planguage gate on non-functional requirements. Every NFR must be quantified:"
  printf '%s\n' "usage: bash nfr-lint.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/nfr-report.json - fix recipes: walteur-kit/REMEDIATION.md (## nfr-lint)"
  printf '%s\n' "bypass: WALTEUR_NFR=off (recorded, not free)"
  exit 0 ;;
esac
