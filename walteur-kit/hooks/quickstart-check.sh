...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "quickstart-check - DETECT-OR-SKIP. Proves the READMEs quickstart actually works."
  printf '%s\n' "usage: bash quickstart-check.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/quickstart-report.json - fix recipes: walteur-kit/REMEDIATION.md (## quickstart-check)"
  printf '%s\n' "bypass: WALTEUR_QUICKSTART=off (recorded, not free)"
  exit 0 ;;
esac
