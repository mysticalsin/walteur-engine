...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "tool-contract-lint - ZERO-DEP HARD gate on agent tool contracts."
  printf '%s\n' "usage: bash tool-contract-lint.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/tool-contract-report.json - fix recipes: walteur-kit/REMEDIATION.md (## tool-contract-lint)"
  printf '%s\n' "bypass: WALTEUR_TOOL_CONTRACT=off (recorded, not free)"
  exit 0 ;;
esac
