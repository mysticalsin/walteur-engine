...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "observe-probe - DETECT-OR-SKIP live-service observability probe."
  printf '%s\n' "usage: bash observe-probe.sh [<base-url>|--help]   (no --selftest: this hook has none)"
  printf '%s\n' "report: walteur-kit/observe-probe-report.json - fix recipes: walteur-kit/REMEDIATION.md (## observe-probe)"
  printf '%s\n' "bypass: WALTEUR_OBSERVE_PROBE=off (recorded, not free)"
  exit 0 ;;
esac

set -uo pipefail

# Argument guard. --help above advertised a --selftest this hook does not implement, and $1 is the
# positional <base-url> — so `observe-probe.sh --selftest` was probed AS A URL ("observe-probe @
# --selftest ... unreachable"), recorded a SKIP and exited 0. An operator asking for a selftest got
# a green that proved nothing. Dash-led tokens are never valid base URLs; fail loudly instead.
case "${1:-}" in
  -*) printf '%s\n' "observe-probe: unrecognized option '$1' — \$1 is the positional <base-url>; this hook implements --help only and has no selftest." >&2
      printf '%s\n' "usage: bash observe-probe.sh [<base-url>|--help]   (or set WALTEUR_PROBE_URL)" >&2
      exit 2 ;;
esac
