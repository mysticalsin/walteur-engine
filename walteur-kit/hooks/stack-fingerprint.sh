...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "stack-fingerprint - manifest-hash drift detector (v9.2 quick win). Deterministic, silent-when-unchanged."
  printf '%s\n' "usage: bash stack-fingerprint.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/stack-fingerprint-report.json - fix recipes: walteur-kit/REMEDIATION.md (## stack-fingerprint)"
  printf '%s\n' "bypass: WALTEUR_STACKFP=off (recorded, not free)"
  exit 0 ;;
esac
