...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "intent-trace - the DETERMINISTIC structural arm of -5.5 (intended-vs-implemented)."
  printf '%s\n' "usage: bash intent-trace.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: see hook header - fix recipes: walteur-kit/REMEDIATION.md (## intent-trace)"
  printf '%s\n' "bypass: WALTEUR_INTENT_TRACE=off (recorded, not free)"
  exit 0 ;;
esac
