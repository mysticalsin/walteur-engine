...
case "${1:-}" in
  -h|--help)
  printf '%s\n' "workflow-quality-gate - class-specific quality proof for the workflow build class."
  printf '%s\n' "usage: bash workflow-quality-gate.sh [--selftest|--help|<default run>]"
  printf '%s\n' "report: walteur-kit/workflow-quality-report.json - fix recipes: walteur-kit/REMEDIATION.md (## workflow-quality-gate)"
  printf '%s\n' "bypass: WALTEUR_WORKFLOW_QUALITY=off (recorded, not free)"
  exit 0 ;;
esac
