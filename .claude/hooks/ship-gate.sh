...
# Fail-SAFE detection: fire on any command mentioning git AND a commit-CREATING verb. This catches
# indirection like `g=git; $g commit` that a literal substring match would skip. Over-triggering only
# ADDS a gate to an unrelated command; it must never let a real ship transition through.
#
# 2026-07-25 — panel-12 REFUTED the "can never let a real ship transition through" claim above, twice,
# with captured exit codes. Both holes are closed here.
#
# HOLE 1: the verb set was only `commit|tag`, so five other commit-CREATING commands plus push sailed
# past with rc=0: cherry-pick, merge --no-ff, revert, am, rebase --continue, push. Each either writes a
# commit or publishes one, which is exactly a ship transition. Verb set widened below.
#
# HOLE 2 (worse): jq was a HARD dependency of the guard and its absence failed OPEN and SILENTLY.
# With jq off PATH, `CMD` came back empty, the grep did not match, and the hook exited 0 with NOTHING on
# stderr — so the entire ~150-gate ship battery was unenforced and no one could tell. A guard whose
# absent dependency yields zero enforcement is not fail-closed. Now: a POSIX fallback extracts the
# command without jq, and if the payload is non-empty yet NO extraction succeeds, the hook BLOCKS
# loudly rather than waving the commit through.
